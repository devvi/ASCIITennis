# Issue 174 — 道具可用性修正: Item Usability Fix Design
...
3. Item blink uses `Date.now()` wall-clock instead of frame counter (`render.js:456`)
4. No P2 item HUD
5. Multi-ball `second_ball` spawned with no proper initialization

## Data Structures (unchanged from existing)
...
| `update_playing()` | `main.js:555-566` | Spawn `second_ball` properly on MULTI_BALL use |
...
if (input1.pressed(BTN_X)) {
  ...
  // FIRE: handled implicitly via fire_mult check at hit time (line 464)
}
```
...
```js
render.item_box(item, frame);
```

### P2 Item HUD (render.js:226-230)

Pass `p2_item` to render options:
```js
if (opts.p2_item) {
...
| `src/constants.js` | none | No changes needed |
